...
class LeaderBoard {
   ...
   * @param callback
   */
  getLeaderboard(group, period, attrName, from, to, callback) {
    let key = this.getPeriodKeyPrefix.call(this, group, period, HASH_TABLE_LEARDER_BOARD_KEY_PREFIX) + attrName;
   ...
   * @param callback
   */
  getRank(group, period, userId, attrName, callback) {
    let key = this.getPeriodKeyPrefix.call(this, group, period, HASH_TABLE_LEARDER_BOARD_KEY_PREFIX) + attrName;
   ...
   * @param callback
   */
  getAroundUserLeaderboard(group, period, userId, attrName, range, callback) {
    let self = this;
