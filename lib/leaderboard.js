...
class LeaderBoard {
  constructor() {
    ...
    this.addHightScore = (userId, attrName, scoreId, score, options) => {
      ...
      let hightScoreKeyPrefix = HASH_TABLE_HIGHT_SCORE_KEY_PREFIX + group + ':';
      let leaderBoardKeyPrefix = HASH_TABLE_LEARDER_BOARD_KEY_PREFIX + group + ':';
     ...
     * @returns {string}
     */
    this.getPeriodKeyPrefix = (group, period, prefix, excludeAllTime) => {
      excludeAllTime = excludeAllTime || false;
      prefix = prefix + group + ':';
      if (period === 'daily') {
